...
class game {
  ...
  updateChXY() {
    ...
    if (Xfocus < 0) Xfocus = bg_im.width / 2 + 22;
    if (Xfocus > bg_im.width / 2 + 22) Xfocus = 0;
    if (Yfocus < 0) Yfocus = bg_im.height / 2 + 60;
    if (Yfocus > bg_im.height / 2 + 60) Yfocus = 0;
  }
    ...
    for (let i = 0; i < FOOD.length; i++)
      if (Math.sqrt((mySnake[0].v[0].x - FOOD[i].x) * (mySnake[0].v[0].x - FOOD[i].x) + (mySnake[0].v[0].y - FOOD[i].y) * (mySnake[0].v[0].y - FOOD[i].y)) > sizeMap) {
        FOOD[i] = new food(
          ...
          (Math.random() - Math.random()) * sizeMap + mySnake[0].v[0].x,
          (Math.random() - Math.random()) * sizeMap + mySnake[0].v[0].y
        );
        // console.log(FOOD[i]);
      }
  }
  ...
  checkDie() {
    for (let i = 0; i < mySnake.length; i++)
      for (let j = 0; j < mySnake.length; j++)
        if (i != j) {
          let kt = true;
          for (let k = 0; k < mySnake[j].v.length; k++) if (this.range(mySnake[i].v[0].x, mySnake[i].v[0].y, mySnake[j].v[k].x, mySnake[j].v[k].y) < mySnake[i].size) kt = false;
          if (!kt) {
  ...
  draw() {
    ...
    for (let i = 0; i < mySnake.length; i++) mySnake[i].draw();
    this.drawScore();
  }

  drawScore() {
    let data = [];
    for (let i = 0; i < mySnake.length; i++) data[i] = mySnake[i];
    ...
    let index = 0;
    for (let i = 1; i < mySnake.length; i++) if (data[i].name == "HaiZuka") index = i;
    this.context.font = this.getSize() / 4 + "px Arial Black";
    for (let i = 0; i < 10; i++) {
      this.context.fillStyle = "#AA0000";
      if (i == index) this.context.fillStyle = "#CC99FF";
  ...
  isPoint(x, y) {
    if (x - XX < -3 * this.getSize()) return false;
    if (y - YY < -3 * this.getSize()) return false;
    if (x - XX > game_W + 3 * this.getSize()) return false;
    if (y - YY > game_H + 3 * this.getSize()) return false;
    return true;
  }
}
